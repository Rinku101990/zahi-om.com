...
class Product_Model extends MY_Model{	 
	...
	function GetList_featured($select,$fld_id,$vnd_fld_id,$vnd_id,$table){
	     $this->db->select($select);	       
		  $this->db->order_by($fld_id,"DESC");
		  $this->db->where($vnd_fld_id,$vnd_id);
	...
	function GetList_trending($select,$fld_id,$vnd_fld_id,$vnd_id,$table){
	     $this->db->select($select);	       
		  $this->db->order_by($fld_id,"DESC");
		  $this->db->where($vnd_fld_id,$vnd_id);
	 ...
	 function brd_list($vnd_id,$table){	
	        $array = array('0', $vnd_id);
	...
	function policy_list($table){		    
	 ...
	 function option_list($vnd_id,$table){	
	        $array = array('0', $vnd_id);
	 ...
	 function option_gorup_list($vnd_id){
	 	 $array = array('0', $vnd_id);
		  $this->db->order_by('opt_id',"asc");
		  $this->db->where('opt_status','1');			
		  $this->db->where_in('vnd_id',$array);					 		
...
public function get_inventry($fld,$id,$tabel)
	{
...
function single_product($fld,$id,$tabel)
 ...
 function get_single($select,$fld,$id,$tabel)
	...
	function scate_export_list($select,$vnd_id,$fld_id,$remove,$status,$table)
	{	
		 $array = array('0', $vnd_id);
	      $this->db->select($select);
		  $this->db->order_by($fld_id,"asc");
	...
	function Product_export_list($vnd_id,$table)
	{	
	...
	function export_list($fld_id,$vnd_id_name,$vnd_id,$table)
	{	
		  $this->db->order_by($fld_id,"DESC");
		  $this->db->where($vnd_id_name,$vnd_id);		
		  $query=$this->db->get($table);
		   if($query->num_rows() ==''){
				return '';
				}else{
...
function watermark_image($file_path,$path){
 $this->load->library('image_lib');
 $new_path=FCPATH .'assets/fonts/texb.ttf';
