		...
		$.ajax({
			...
			success:function(response){
				if(response!=''){
					...
					$("#intsize").val(response.inventory.int_size);
					$.each(response.color, function(key, value ) 
					{
					if(response.inventory.int_color==value){
					var option = $('<option selected>').attr('value', value).html(value);
					}else{
					var option = $('<option>').attr('value', value).html(value);	
					}
					$('#int_color').append(option);
					});	
					$("#selleing_price").val(response.inventory.int_selleing_price);
					$("#int_total_qty").val(response.inventory.int_total_qty);
