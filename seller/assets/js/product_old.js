...
//         else{ $('#Weight').html(' ');$('.pd_weight').css('border',''); check = true; }
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/General_Product_save",
...
//         else{ $('#Weight').html(' ');$('.pd_weight').css('border',''); check = true; }
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/General_Product_Update",
				data:$('#GeneralProductForm').serialize(),
				dataType:'json',				
				beforeSend: function ()
				{
   ...
   $(".general_product_prev").click(function()
	...
	$(".CategoryProduct").click(function()
	{
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/Category_Product_save",
	...
	$(".CategoryProduct_update").click(function()
	{
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/Category_Product_Update",
				data:$('#CategoryProductForm').serialize(),
	...
	$(".OptionProduct").click(function()
	{
       ...
       let PID 		= $('.p_id').val();
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/Option_Product_save",
	...
	$(".OptionProduct_update").click(function()
	{
       ...
       let PID 		= $('.p_id').val();
		...
		if(check){
			...
			{	
				type: "POST",
				url: url+"product/Option_Product_update",
				data:$('#OptionGroupProductForm').serialize(),
				dataType:'json',
  ...
  //       else{ $('#purchase_qty').html(' ');$('.int_min_purchase_qty').css('border',''); Pcheck = true; }  
		...
		if(check && Pcheck){
			...
			{	
				type: "POST",
				url: url+"product/Inventory_Product_Save",
  ...
  //       else{ $('#purchase_qty').html(' ');$('.int_min_purchase_qty').css('border',''); Pcheck = true; }  
		...
		if(check && Pcheck){
			...
			{	
				type: "POST",
				url: url+"product/Inventory_Product_update",
				data:$('#InventoryProductForm').serialize(),
				
				beforeSend: function ()
				{
					$('.Inventory_update').html('Update...');
					$('.Inventory_update').prop('disabled', true);
				},
				success: function(response)
				{
					if(response == 'Failed'){
	...
	$(".WarrantyProduct").click(function()
	{
        ...
        $.ajax(
			{	
				type: "POST",
				url: url+"product/warranty_product_Save",
	...
	$(".ReturnProduct").click(function()
	{
        ...
        $.ajax(
			{	
				type: "POST",
				url: url+"product/return_product_Save",
	...
	$(".SpecialPriceProduct").click(function()
	{
       ...
       if(check && Pcheck){
			...
			{	
				type: "POST",
				url: url+"product/Special_Price_Product_Save",
	...
	$(".SpecialPrice_update").click(function()
	{
       ...
       if(check && Pcheck){
			...
			{	
				type: "POST",
				url: url+"product/Special_Price_Product_update",
				data:$('#SpecialPriceProductForm').serialize(),
	...
	$(".DiscountProduct").click(function()
	{
        ...
        if(check){
			...
			{	
				type: "POST",
				url: url+"product/Discount_Product_Save",
	...
	$(".Discount_update").click(function()
	{
        ...
        if(check){
			...
			{	
				type: "POST",
				url: url+"product/Discount_Product_update",
				data:$('#DiscountProductForm').serialize(),
	...
	$(".ImageProduct").click(function()
	{
        ...
        if(check){
			...
			{	
				type: "POST",
				url: url+"product/Image_Product_Save",
		     	data: formData,
				processData: false,
                contentType: false,
	...
	$(".ImageProduct_update").click(function()
	{
        ...
        if(check){
			...
			{	
				type: "POST",
				url: url+"product/Image_Product_update",
		     	data: formData,
				processData: false,
                contentType: false,
   ...
   let maxFieldq = 15; //Input fields increment limitatio	
    ...
    function add_size(e) {
          let url 		= $('#site_url').val();	
			...
			{	
				type: "POST",
				url: url+"product/inventry_size_filed",
				data:$('#OptionGroupProductForm').serialize(),
				dataType:'json',
				...
				// $(".InventoryResponse").html('<div class="alert alert-success alert-dismissible fade show" role="alert"> <span class="alert-inner--icon"><i class="fe fe-thumbs-up "></i></span> <span class="alert-inner--text"><strong>Success!</strong> Option Product has been successfully Save.</span> <button type="button" class="close text-white" data-dismiss="alert" aria-label="Close"> <span aria-hidden="true">×</span> </button> </div>');
                $('#size_field').append('<span id="sizefield'+k+'" style="    display: inline-flex;">'+response.option+'<div class="col-sm-2 col-md-2"><label>&nbsp;</label><a href="javascript:void(0);" class="btn btn-default  pull-right"  onclick="remove_size('+k+',this)" style="    margin-top: 30px;" ><i class="fa fa-close"></i> Remove</a></div></span>');
...
function GetCateRemove(e) {
     ...
     let category 	=   $(e).attr('category');
		    ...
		    $.ajax({
            type: "POST",
            url: url+"product/category_remove",
...
function subcategory(SID,url,clt) {	
    ...
    else
    {
        $.ajax({
            type: "POST",
            url: url+"product/category_search",
...
$(".inventory_edit").click(function(){
  ...
  var int_id = $(this).attr("inventory_edit");
		$.ajax({
			method:'post',
			url:refer_url+'product/inventory_edit',
			data:{int_id:int_id},
			dataType:'json',
			success:function(response){
				if(response!=''){
...
$(".ProductView").click(function(){
  ...
  var pid = $(this).attr("ProductView");
		$.ajax({
			method:'post',
			url:refer_url+'product/ProductView',
			data:{pid:pid},
			dataType:'json',
			success:function(response){
				if(response!=''){					
