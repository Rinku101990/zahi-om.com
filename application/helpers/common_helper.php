...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
   ...
   function currency($id){ 
	...
	function seller(){ 
		$ci=get_instance();		
		$ci->db->select('vnd_id,vnd_name,vnd_name_ar');
		$ci->db->order_by('vnd_id', 'DESC');
		$ci->db->where('vnd_status', '1');
		$ci->db->where('vnd_VerifiedStatus', '1');
        $query = $ci->db->get('tbl_vendor');
		if($query->num_rows() ==''){
				return false;
				}else{
	...
	function vendor_phone($id){ 
		$ci=get_instance();		
		$ci->db->select('vnd_id,vnd_name,vnd_phone,vnd_phone_code');		
		$ci->db->where('vnd_id', $id);		
        $query = $ci->db->get('tbl_vendor');
		if($query->num_rows() ==''){
				return false;
	...
	function navigation(){ 
		$ci=get_instance();
		$ci->db->select('mn_id,mn_name,mn_slug,mn_name_ar,mn_category_id');
		$ci->db->order_by('mn_order','asc');	
		$ci->db->where('mn_status', '1');	
	...
	function get_category(){ 
		$ci=get_instance();
		$ci->db->select('cate_id,cate_name,cate_name_ar,cate_slug,cate_img,cate_icon');
		$ci->db->order_by('cate_id','asc');	
  ...
  function category_list(){ 
	...
	function get_brand(){ 
		$ci=get_instance();	
		$ci->db->select('brd_id,brd_name');	
		$ci->db->order_by('brd_name', 'ASC');
        $query = $ci->db->get('tbl_brand');
		if($query->num_rows() ==''){
				return false;
				}else{
	...
	function get_inventory($id){ 
		$ci=get_instance();
		$ci->db->select('int_available_qty');
		$ci->db->where('int_id', $id);
		$query = $ci->db->get('tbl_inventory');       
		if($query->num_rows() ==''){
				return false;
				}else{
	...
	function inventory_size($id){ 
		$ci=get_instance();
		$ci->db->select('int_id,int_size');
		$ci->db->where('int_pid', $id);
		$ci->db->group_by('int_size');
		//$ci->db->where('opt_status', '1');
        $query = $ci->db->get('tbl_inventory');
	...
	function inventory_color($id){ 
		$ci=get_instance();
		$ci->db->select('int_id,int_color');
		$ci->db->group_by('int_color');
		$ci->db->where('int_pid', $id);
		//$ci->db->where('opt_status', '1');
        $query = $ci->db->get('tbl_inventory');
       // echo $ci->db->last_query($query);
	...
	function tax($id){ 
		$ci=get_instance();	
		$ci->db->select('tx.txt_name,tx.txt_per');
		$ci->db->from('tbl_product tp');
		$ci->db->join('tbl_tax tx','tx.cate_id=tp.p_cate','LEFT');
		$ci->db->where('tp.p_id', $id);
		$ci->db->where('tx.txt_status', '1');
        $query = $ci->db->get();
		if($query->num_rows() ==''){
				return 'Admin';
				}else{
...
function tax_name($id){ 
	...
	function delivery_st_charge($id){ 
		$ci=get_instance();	
		$ci->db->select('min_amount,amount');	
   ...
   function getVnd_name($id){ 
	...
	function discount($id,$amount,$qty){ 
			$ci=get_instance();	
			$ci->db->select('vd_min_purchase_qty,vd_discount');			
			$ci->db->where('vd_pid', $id);
	        $query = $ci->db->get('tbl_volume_discount');
			if($query->num_rows() ==''){
					return false;
					}else{
				//return $query->row();
 ...
 function discount_per($id){ 
	...
	function cancel_item($pid,$ordid){ 
			$ci=get_instance();				
			$ci->db->where('c_pro_id', $pid);			
			$ci->db->where('c_order_id', $ordid);			
	        $query = $ci->db->get('tbl_cancel_item');
	       // $ci->db->last_query($query);	       
			if($query->num_rows() ==1){
				return $query->num_rows();
					}else{
