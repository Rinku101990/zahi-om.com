  	...
  	public function index()
	{
		...
		if(!empty($ordid))
		{
		   if(empty($this->session->userdata('sleeve'))){	
			...
			$orderUpdate = array('ord_txt_status' => 'success','order_status' => 'Waiting','status' => '2');
		...
		//	$content['subview']="thankyou/viewMyOrder"; 
			$this->load->view('thankyou/viewMyOrder', $content);
  ...
  public function success()
	{
	   ...
	   if(!empty($this->session->userdata('session_id')))
        ...
        curl_setopt_array($curl, [
          CURLOPT_URL => "https://checkout.thawani.om/api/v1/checkout/session/$sessionID",
        ...
        if ($err) {
		 ...
		 foreach ($data['cmpt_ord_pro_detail'] as $ord_value) {
		   ...
		   $Updateinventory= $this->Order_Model->Update_inventory($data_invent,$ord_value->int_id);
		   // vendor sms 
			...
			$vndsmsmsg='Dear '.$vnd_name.' , You have received new order #'.$data['cmpt_ord_detail']->ord_reference_no.'.  kindly check and confirm order deliver time and collection. ';
			 $this->load->library('twilio');

				$sms_from = '+447723561455';
				$vndsms_to = $vendorphone;
				$vndsmsresponse = $this->twilio->sms($sms_from, $vndsms_to, $vndsmsmsg);
		  ...
		  //  echo"<pre>";
		  //  print_r($data['cmpt_ord_detail']);
		    /*====================Start sms Customer  send===========*/ 
		  $name=$data['cmpt_ord_detail']->cust_fname.' '.$data['cmpt_ord_detail']->cust_lname;
		   if(!empty($data['cmpt_ord_detail']->cust_phone_code)){
		    $phone='+'.code($data['cmpt_ord_detail']->cust_phone_code).''.$data['cmpt_ord_detail']->cust_phone;
			$smsmsg='Dear '.$name.' , We have received your order #'.$data['cmpt_ord_detail']->ord_reference_no.' & we will provide your tracking number once your order is shipped.';
			 $this->load->library('twilio');
		...
		$smsresponse = $this->twilio->sms($sms_from, $sms_to, $smsmsg);
		 ...
		 /*====================Start sms admin send===========*/  
		
		$adminsmsmsg='New order placed by '.$name.' with order number #'.$data['cmpt_ord_detail']->ord_reference_no.'.';
		$this->load->library('twilio');
		$sms_from = '+447723561455';
		$adminsms_to = '+96893993412';
		$smsresponse = $this->twilio->sms($sms_from, $adminsms_to, $adminsmsmsg);		
			/*====================End sms send ===========*/
	   ...
	   if(!empty($this->session->userdata('session_id')))
        ...
        if ($err) {
          echo "cURL Error #:" . $err;
			...
			$orderUpdate = array('ord_txt_status' => 'cancel','order_status' => 'Cancel','status' => '2');

		    $success = $this->Order_Model->UpdateOrderInformation($content['order_detail']->ord_reference_no,$orderUpdate,$this->table_orders);
		    
     		$this->cart->destroy();
			//$content['subview']="thankyou/viewCancel"; 
			$this->load->view('thankyou/viewCancel', $content);
        }
     }else{
			redirect('/');
		}
