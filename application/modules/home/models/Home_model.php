...
class Home_model extends MY_Model{
	...
	public function cate_cate_name($id,$table){		
		$this->db->select('cate_name as category,cate_name_ar as category_ar,cate_name as scategory,cate_name_ar as scategory_ar,cate_name as mcategory,cate_name_ar as mcategory_ar,cate_meta_title,cate_meta_keyword,cate_meta_description');
		$this->db->from($table);		
	...
	public function cate_vendor_name($id,$table){		
		$this->db->select('vnd.vnd_name as category,vnd.vnd_name as category_ar,vnd.vnd_name as scategory,vnd.vnd_name as scategory_ar,vnd.vnd_name as mcategory,vnd.vnd_name as mcategory_ar,brd.cate_meta_title,brd.cate_meta_keyword,brd.cate_meta_description');
		$this->db->join('tbl_brand brd','brd.vnd_id=vnd.vnd_id','LEFT');
		$this->db->from($table. ' vnd');		
		$this->db->where('vnd.vnd_id',$id);			
		$query=$this->db->get();
		// echo $this->db->last_query($query);
		// die();
		if($query->num_rows() != 0) return $query->row();
	...
	public function cate_sub_name($id,$table){		
		$this->db->select('scate.scate_name as category,scate.scate_name_ar as category_ar,scate.scate_name as scategory,scate.scate_name_ar as scategory_ar,cate.cate_name as mcategory,cate.cate_name_ar as mcategory_ar,scate.cate_meta_title,scate.cate_meta_keyword,scate.cate_meta_description');	
	...
	public function cate_child_name($id,$table){		
		$this->db->select('cld.child_name as category,cld.child_name_ar as category_ar,scate.scate_name as scategory,scate.scate_name_ar as scategory_ar,cate.cate_name as mcategory,cate.cate_name_ar as mcategory_ar,cld.cate_meta_title,cld.cate_meta_keyword,cld.cate_meta_description');	
...
function categories_product_list($fld_val,$table){
	...
	function get_eid_product_list($table){
		$this->db->select('cate.cate_slug,scate.scate_slug,child.child_slug,tp.p_id,tp.p_cate,tp.p_scate,tp.p_child,tp.p_reference_no,tp.p_name,tp.p_model,tp.p_selling_price,tp.p_short_description,sp.sp_special_price,sp.sp_end_date,pimg.pg_image');	
		$this->db->order_by('tp.p_id',"DESC");		
		$this->db->join('tbl_special_price sp','sp.sp_pid=tp.p_id','LEFT');
		//$this->db->join('tbl_brand tb','tb.brd_id=tp.p_brand','LEFT');
...
function make_query($brand,$color,$size,$price,$grade,$condition,$category,$category2,$sub_category,$sub_category2,$child_category,$hot,$featured,$trending,$eid,$supplier,$newest_first)
 { 
  ...
  if(isset($condition))
	 {
  $query = "SELECT `tp`.`p_id`, `tp`.`p_cate`, `tp`.`p_scate`, `tp`.`p_child`, `tp`.`p_reference_no`, `tp`.`p_name`,`tp`.`p_name_ar`, `tp`.`p_model`, `tp`.`p_selling_price`, `tp`.`p_short_description`, `sp`.`sp_end_date`,`cate`.`cate_name`, `scate`.`scate_name`, `child`.`child_name`, `sp`.`sp_special_price`,`sp`.`sp_end_date`,`sp`.`sp_start_date`, `pimg`.`pg_img`,`brd`.`brd_name` as `vnd_name` FROM `tbl_product` `tp` LEFT JOIN `tbl_special_price` `sp` ON `sp`.`sp_pid`=`tp`.`p_id` LEFT JOIN `tbl_product_img` `pimg` ON `pimg`.`pg_pid`=`tp`.`p_id` LEFT JOIN `tbl_category` `cate` ON `cate`.`cate_id`=`tp`.`p_cate` LEFT JOIN `tbl_sub_category` `scate` ON `scate`.`scate_id`=`tp`.`p_scate` LEFT JOIN `tbl_child_category` `child` ON `child`.`child_id`=`tp`.`p_child` LEFT JOIN `tbl_product_link` `pl` ON `pl`.`p_id`=`tp`.`p_id` LEFT JOIN `tbl_inventory` `int` ON `int`.`int_pid`=`tp`.`p_id` LEFT JOIN `tbl_brand` `brd` ON `brd`.`brd_id`=`tp`.`p_brand` WHERE `tp`.`p_status` = '1' AND `tp`.`p_approval_status` = '0' 
    ";  
...
function product_data($page,$start,$brand,$color,$size,$price,$grade,$condition,$category,$category2,$sub_category,$sub_category2,$child_category,$hot,$featured,$trending,$eid,$supplier,$newest_first)
 ...
 //$start = ceil($page *  $product_limit);
  ...
  {
  	$date=date('Y-m-d');
  	$output .= '<div class="tab-pane fade show active" id="large" role="tabpanel" style="width:100%">
                                    <div class="row cate_tab_product">';
	 foreach($data->result_array() as $row){
	
       if(!empty($row['pg_img'])){
        $img='<img src="'.base_url('seller/uploads/').slug($row['cate_name']).'/'.slug($row['scate_name']).'/'.slug($row['child_name']).'/'.$row['pg_img'].'" alt="'.$row['p_name'].'" class="img-fit lazyloaded">';
         }else{
    ...
    $website=$this->get_website('web_id','tbl_website_info');
         if($website->web_lang=='en'){
                        	$pname=$row['p_name'];
                        }else{ if(!empty($row['p_name_ar'])){$pname=$row['p_name_ar'];}else{$pname=$row['p_name'];}}
  ...
  {
   $output = '<div class="col-md-12"><img src="'.base_url('assets/img/not-available.gif').'" style="width: 55%; margin: 10% 25%;"></div>';
  }
  return $output; }
