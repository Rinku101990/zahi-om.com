	...
	function product_data()
	 {
		  ...
		  echo $query = $this->Home_Model->product_data($page,$brand,$color,$size,$price,$grade,$condition,$category,$category2,$sub_category,$sub_category2,$child_category,$hot,$featured,$trending,$supplier,$newest_first);
     		...
     		echo'<div class="typed-search-box"><div id="search-content">
          	...
          	foreach ($product as $row):   
       			if(!empty($row->pg_image)){
        			$img='<img src="'.base_url('seller/uploads/').slug($row->cate_name).'/'.slug($row->scate_name).'/'.slug($row->child_name).'/'.explode(',',$row->pg_image)[0].'" title="'.$row->p_name.'" style="object-fit: contain;width:50px !important; height: 50px !important;">';
           		}else{
        			$img='<img src="'.base_url('seller/uploads/default-image.png').'" title="'.$row->p_name.'" style="object-fit: contain;width:70px !important; height: 70px !important;">';
         		}
         		if(!empty($row->sp_special_price) && $row->sp_start_date <= $date && $row->sp_end_date >= $date){
         			$price='<del class="old-product-price strong-400">'.currency($this->website->web_currency).''.number_format($row->p_selling_price).'</del>
             			<span class="product-price strong-600">'.currency($this->website->web_currency).''.number_format($row->sp_special_price).'</span>';
        		}else{
        			$price='<span class="product-price strong-600">'.currency($this->website->web_currency).''.number_format($row->p_selling_price).'</span>';
       			}  
