...
class Payment extends MY_Controller {
    ...
    public function index()
	{
		if(!empty($this->cart->contents())){
			...
			$this->load->view('payment/viewMakePayment', $content);
	...
	public function check_coupon_value($cou_code,$amount){
		...
		if($check != false){
			...
			if($check->cup_type == 'flat'){
			return $discount = $check->cup_discount;
			}else if($check->cup_min_order <= $amount && $check->cup_type == 'Percentage'){
			return $discount =  $check->cup_discount*$amount/100;
			//echo $check->cup_discount;
			}else{
			return $discount =  0;
		           }
	...
	public function placeOrder(){
        ...
        foreach($this->cart->contents() as $data)
				{
		if(vndcountry($data['vnd'])==$ship_Address->cnt_id){
                                   $get_ship+=0;
                                  }else{ $get_ship+=1;} 
                              }

           if(empty($get_ship)){
          if($this->website->web_local_free_shipping >=$this->cart->total()){
           $dl_charges=$this->website->web_local_shipping;
       }else{$dl_charges=0;}
       }else{$dl_charges=$this->website->web_national_shipping;}
// 		if(!empty(delivery_st_charge($ship_Address->st_id)->min_amount)){
...
// 				}else{$dl_charges='0';}	
		...
		$cou_code=$this->input->post('cou_code');
		$gettotal=$this->cart->total()+$dl_charges;
		$checkRs=$this->check_coupon_value($cou_code,$gettotal);
	// 	$check=$this->Payment_Model->check_coupon($cou_code,$this->table_coupon);
	// if(!empty($check->cup_discount)){
	// 	$checkRs=($this->cart->total()+$dl_charges)*$check->cup_discount/100;}
	// 	else{$checkRs=0;}
		...
		{ 
			...
			$orderReferenceID = $year.$month.$date.$time; /*-- Order Reference Number --*/ 
			...
			if($ordid){				
				...
				{ $vnd=$data['vnd'];
					...
					$getorderProducts[] = array(
						...
						'unit_amount' => $ord_adj_amount*1000,						
						'quantity' => '1'
				...
				// 	);
						...
						//$getorderProducts_array=array_merge($getorderProducts);
						// echo"<pre>";
						// print_r($getorderProducts_array);
						// die;
					$this->thawani_pay($orderReferenceID,$getorderProducts,$name,$cust_email,$cust_phone,$ordid);
 ...
 function thawani_pay($orderReferenceID,$getorderProducts,$name,$cust_email,$cust_phone,$ordid){

// echo"<pre>";
// print_r($getorderProducts);die;
 		$arr['client_reference_id'] = $orderReferenceID;
                        $arr['customer_id'] = '';
                         $arr['products']=$getorderProducts;
                        //  $arr['total_amount']=3000;
                         ...
                         $curl = curl_init();
						    curl_setopt_array($curl, [
						      ...
						      CURLOPT_HTTPHEADER => [
						        ...
						        "thawani-api-key: alU2hqn5MGvldfN5V71SKPRow3BIf3"
						      ],
						    ]);
